...
{
	auto flareMat = mMaterials["flare"].get();

	float theta = gt.TotalTime();

	XMMATRIX translation = XMMatrixTranslationFromVector(XMVectorSet(-0.5f, -0.5f, 0.0f, 0.0f));

	XMMATRIX rotation = XMMatrixRotationZ(theta);

	XMStoreFloat4x4(&flareMat->MatTransform ,XMMatrixMultiply(XMMatrixMultiply(translation, rotation), -translation));


	/*flareMat->MatTransform(0, 0) = cos(theta);
	flareMat->MatTransform(0, 1) = sin(theta);
	flareMat->MatTransform(1, 0) = -sin(theta);
	flareMat->MatTransform(1, 1) = cos(theta);
*/
