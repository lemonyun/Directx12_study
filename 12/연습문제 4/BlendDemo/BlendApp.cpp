...
void BlendApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetPipelineState(mPSOs["test"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Transparent]);
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Opaque]);
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::AlphaTested]);
...
void BlendApp::BuildShadersAndInputLayout()
{
	...
	mShaders["normalVS"] = d3dUtil::CompileShader(L"Shaders\\Normal.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["normalPS"] = d3dUtil::CompileShader(L"Shaders\\Normal.hlsl", nullptr, "PS", "ps_5_0");
	mShaders["normalGS"] = d3dUtil::CompileShader(L"Shaders\\Normal.hlsl", nullptr, "GS", "gs_5_0");
...
void BlendApp::BuildPSOs()
{
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC testPsoDesc = opaquePsoDesc;
	testPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["normalVS"]->GetBufferPointer()),
		mShaders["normalVS"]->GetBufferSize()
	};
	testPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["normalPS"]->GetBufferPointer()),
		mShaders["normalPS"]->GetBufferSize()
	};
	testPsoDesc.GS =
	{
	...
	testPsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT;
