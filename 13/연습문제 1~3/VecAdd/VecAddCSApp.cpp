...
void VecAddCSApp::BuildBuffers()
{
	...
	for(int i = 0; i < NumDataElements; ++i)
	{
		dataA[i].v1 = XMFLOAT3(rand() % 3 + 1, rand() % 3 + 1, rand() % 3 + 1);
	}
...
void VecAddCSApp::BuildRootSignature()
{
    ...
    slotRootParameter[0].InitAsDescriptorTable(1, &uavTable1);
	slotRootParameter[1].InitAsDescriptorTable(1, &uavTable2);
...
void VecAddCSApp::BuildDescriptorHeaps()
{
	...
	uavDesc.Buffer.StructureByteStride = 12;
