...
void BlendApp::AnimateMaterials(const GameTimer& gt)
{
	...
	int framecnt = (gt.TotalTime() - (int)gt.TotalTime()) / (1.0f / 60.0f);
	
	boltMat->DiffuseSrvHeapIndex = 3 + framecnt;
...
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData cylinder = geoGen.CreateCylinder(8.0f, 8.0f, 8.0f, 16, 8);
...
void BlendApp::BuildRenderItems()
{
	...
	mRitemLayer[(int)RenderLayer::Additive].push_back(cylinderRitem.get());
