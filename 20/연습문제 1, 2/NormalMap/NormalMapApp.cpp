    ...
    : D3DApp(hInstance)
{
	...
	mSceneBounds.Radius = sqrtf(5.0f * 5.0f + 7.5f * 7.5f);
}
...
void NormalMapApp::UpdateShadowTransform(const GameTimer& gt)
{
	...
	XMMATRIX lightProj = XMMatrixPerspectiveFovLH(0.25f * MathHelper::Pi, 1.0f, n, f);
...
void NormalMapApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.Lights[0].FalloffEnd = 2.0f;
	mMainPassCB.Lights[0].FalloffStart = 1.0f;
	mMainPassCB.Lights[0].SpotPower = 5.0f;
	...
	mMainPassCB.EyePosW = mLightPosW;
	mMainPassCB.RenderTargetSize = XMFLOAT2((float)w, (float)h);
	mMainPassCB.InvRenderTargetSize = XMFLOAT2(1.0f / w, 1.0f / h);
...
void NormalMapApp::BuildRootSignature()
{
	...
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 11, 1, 0);
...
void NormalMapApp::BuildDescriptorHeaps()
{
	...
	std::vector<ComPtr<ID3D12Resource>> tex2DList = 
	{
		...
		mTextures["boxDiffuseMap"]->Resource
	};
...
void NormalMapApp::BuildShadersAndInputLayout()
{
	...
	mShaders["projectorVS"] = d3dUtil::CompileShader(L"Shaders\\Projector.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["projectorPS"] = d3dUtil::CompileShader(L"Shaders\\Projector.hlsl", nullptr, "PS", "ps_5_1");
...
void NormalMapApp::BuildPSOs()
{
	...
	opaquePsoDesc.VS = 
	{ 
		reinterpret_cast<BYTE*>(mShaders["projectorVS"]->GetBufferPointer()), 
		mShaders["projectorVS"]->GetBufferSize()
	};
	opaquePsoDesc.PS = 
	{ 
		reinterpret_cast<BYTE*>(mShaders["projectorPS"]->GetBufferPointer()),
		mShaders["projectorPS"]->GetBufferSize()
	};
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> NormalMapApp::GetStaticSamplers()
{
	...
	const CD3DX12_STATIC_SAMPLER_DESC anisotropicWrap(
		...
		D3D12_STATIC_BORDER_COLOR_TRANSPARENT_BLACK);                               // maxAnisotropy
